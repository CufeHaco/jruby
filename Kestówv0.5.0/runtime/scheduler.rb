...
# Kestówv 0.5.0 — runtime/scheduler.rb
#
    ...
    module Scheduler
      ...
      class << self
        ...
        def register_with_boot
          Boot.register(:runtime_scheduler)
          Boot.set_bit(:runtime_scheduler)
          init_run_queues
          self
        end
        ...
        def enqueue(task, cpu_id: best_cpu)
          ...
          rq = run_queue(cpu_id)
          rq.enqueue(task)
          Boot.set_bit(:"sched_task_#{task.tid}")
          true
        end
        ...
        def block_task(task)
          ...
          Boot.clear_bit(:"sched_task_#{task.tid}")
        end
        ...
        def run_idle(cpu_id)
          ...
          Boot.set_bit(:"cpu_idle_#{cpu_id}")

          # Reap any zombies that wait.rb hasn't collected
          reap_zombies

          Boot.clear_bit(:"cpu_idle_#{cpu_id}")
        end
...
Kestowv::Config::Modules.register(
  ...
  __FILE__,
  feature:    :scheduler,
  depends_on: [:runtime_run_queue, :proc_task, :core_signals, :hal_cpu]
)
