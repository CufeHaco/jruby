...
matched = case Boot.byte_dispatch("/tmp/boot_test_syscall.rb") rescue nil
...
assert "pattern match syntax valid", [:matched, :file_gone].include?(matched)
...
Boot.load("/nonexistent_boot_test_file.rb")
assert "custom handler called",       !caught.nil?
assert "handler received error class", caught[:error] <= Exception

# :raise mode
Boot.config.on_error = :raise
raised = false
begin
  Boot.dispatch("/nonexistent_raise_test.rb")
rescue => e
  raised = true
end
