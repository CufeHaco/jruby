  ...
  module Proc
    module Cgroup
      ...
      class CgroupNode
        ...
        def pids_exceeded?
          max = get_limit(:pids, :max)
          return false if max == :unlimited
          @mutex.synchronize { @members.size >= max }
        end
      ...
      class << self
        ...
        def create(name, parent_id: @root_id, controllers: [])
          ...
          Boot.register(cgroup_bit(id))
          Boot.set_bit(cgroup_bit(id))
          node
        end
        ...
        def destroy(id)
          ...
          return false unless node.leaf?   # must be empty of children
          return false unless node.task_count.zero?  # must have no tasks
        ...
        def assign(pid, cgroup_id)
          node = @mutex.synchronize { @cgroups[cgroup_id] }
          return :not_found unless node
          return :pids_exceeded if node.pids_exceeded?
        ...
        def cgroup_for(pid)
          cid = @mutex.synchronize { @task_map[pid] }
          @mutex.synchronize { @cgroups[cid] } if cid
        end
