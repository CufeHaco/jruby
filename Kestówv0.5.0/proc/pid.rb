      ...
      class << self
        ...
        def allocate(task: nil)
          pid = @mutex.synchronize do
            p = @free_list.shift || next_pid_unsafe
            return nil unless p
          ...
          Boot.register(pid_bit(pid))
          Boot.set_bit(pid_bit(pid))
          pid
        end
        ...
        def release(pid)
          return false if RESERVED.include?(pid)
          ...
          Boot.clear_bit(pid_bit(pid)) if removed
          removed
        end
